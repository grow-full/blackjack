...
void printcard(int p, int turns)
{
	...
	if (ace_score == -1)
	{
		printf("ACE가 나왔습니다, [1/11]점으로 계산합니다: ");
		scanf_s("%d", &ace_score);
...
int betting(struct Player player)
{
	...
	printf("베팅을 시작합니다! %d 중 얼마를 배팅하시겠습니까? ", player.cash);
	scanf_s("%d", &bet);

	if (bet > player.cash)
	{
		printf("잔액이 부족합니다. 다시 베팅해주세요 : ");
		scanf_s("%d", &bet);
		return bet;
	}
	else return bet;
...
void play()
{
	...
	printf("게임이 종료됩니다. 지금까지 고생하셨습니다. \n\n\n\n\n");
}

//파일 저장
void save() {
	FILE* txt = fopen("blackjack.txt", "w");
	for (int i = 0; i < 3; i++) {
		fprintf(txt, "플레이어%d : %d,%d \n", i+1,player[i].cash,player[i].score);
	}
	/*fprintf(txt, "%s ", dealer);*/
	printf("저장완료!!");
	fclose(txt);
}
...
void start()
{
	...
	{
		printf(" 게임을 종료합니다. 지금까지 게임에 참여하신 여러분 고생하셨습니다.   \n\n\n\n\n");
		/*save();*/
	}
	return;
}
